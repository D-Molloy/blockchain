...
// uuid() === eg 45745c60-7b1a-11e8-9c9c-2d42b21b1a3e'
...
app.post("/transaction/broadcast", function(req, res) {
  ...
  Promise.all(requestPromises)
    ...
    .catch(err => {
      console.log(err);
      res.json({
        note: "An error occured while broadcasting the transaction."
      });
    });
});
...
// 1 - this route is hit when a non-network node that wants to be added with the data of the new URL.  It is registered on the node that received it, and then broadcast to the other network nodes via the /register-node route
