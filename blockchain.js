...
function Blockchain() {
  ...
  // once validated they are added to the blockchain when a new block is created
  // when createNewTransaction is invoked, the new transaction is push into the pendingTransactions array
...
Blockchain.prototype.createNewBlock = function(nonce, previousBlockHash, hash) {
  //create a new block obj -  all of the data we need for a block
  const newBlock = {
...
//  every time we create a new block, we first must make sure it's legitimate by mining it through proof PoW
...
//find the blocks where the sender or recipient match the address and return the total balance
Blockchain.prototype.getAddressData = function(address) {
  const addressTransactions = [];
  //cycling through every block in the chain
  this.chain.forEach(block => {
    block.transactions.forEach(transaction => {
